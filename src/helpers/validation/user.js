...
const authSchema = Joi.object({
    first_name: Joi.string().required().min(3).max(255).empty().messages({
        "any.required": "Sorry, name is required",
        "string.empty": "name cannot be an empty field",
        "string.min": "name should have a minimum length of 3 and a maximum length of 255",
    }),
    last_name: Joi.string().required().min(3).max(255).empty().messages({
        "any.required": "Sorry, name is required",
        "string.empty": "name cannot be an empty field",
        "string.min": "name should have a minimum length of 3 and a maximum length of 255",
    }),
    address: Joi.string().min(3).max(255).empty().messages({
        "string.min": "name should have a minimum length of 3 and a maximum length of 255",
    }),
    email: Joi.string()
        .required()
